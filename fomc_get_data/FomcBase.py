...
class FomcBase(metaclass=ABCMeta):
    ...
    def import_mongo_pickle(self,client=None,filename='output.pickle'):
        self.client = client
        self.data_pickle = pickle.dumps(self.df)
        fs = gridfs.GridFS(client)
        data = self.client.fs.files.find_one({'filename':filename})
        if data:
            fs.delete(data['_id'])
        fs.put(self.data_pickle,filename = filename)
        print("\nDa them thanh cong:",filename)
